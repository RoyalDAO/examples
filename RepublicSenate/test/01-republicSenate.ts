...
import deployExecutor from "../deploy/01-deploy-republicExecutor";
import deployChancellor from "../deploy/03-deploy-republicChancellor";
...
import deploySenate from "../deploy/02-deploy-republicSenate";
...
describe("RepublicSenateDao", function () {
  ...
  let _republicChancellorContract: RepublicChancellor;
  beforeEach(async function () {
    ...
    const chancelorFactory: RepublicChancellor__factory =
      await ethers.getContractFactory("RepublicChancellor");
    const _chancelor = await get("RepublicChancellor");

    _republicChancellorContract = await chancelorFactory.attach(
      _chancelor.address
    );
  ...
  describe("RepublicSenateDao::SenateTest", function () {
    it("Try to accept new senate member through proposal. It should work with no problems", async function () {
      ...
      for (const minter of _minters) {
        ...
        if (voteCounter > (_minters.length / 4) * 3) {
          voteOpt = 0;
          voteDesc = "is against";
        }
  ...
  async function propose(
    ...
    republicChancelor: RepublicChancellor
  ): Promise<BigNumber> {
    const account = await getAccount("owner");
